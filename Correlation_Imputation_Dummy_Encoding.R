...
# reading data in 
data <- read.csv("trimmed_compiled_data.csv")
...
print(length(imputation_block))
print(length(pm_sorted))
...
# fill int he missing values with the imputed values
xdata_mice_filled <- complete(mice_xdata, 10) 
...
or_data <- read.csv("imputed_data.csv") 
...
for(i in 1:numcol) {
  ...
  if(xtype=='character') {
    a <- colnames(data[i])
    print(paste(a, " is character type")) 
    xdata[[i]] <- as.factor(unlist(data[,i]))}
}
...
# We need to further convert it from factors to numeric 
numcol <- ncol(no_cat_data)
for(i in 1:numcol) {
  xtype <- typeof(no_cat_data[[i]])
  print(xtype)
  if(is.numeric(no_cat_data[[i]]) == F){
    no_cat_data[[i]] <- as.numeric(no_cat_data[[i]])
...
data_filled = data[, !colnames(data) %in% corr_feat]
...
# Last check that there are no remaining missing values
p_missing <- unlist(lapply(data_filled, function(x) sum(is.na(x))))/nrow(data_filled)
pm_sorted <- sort(p_missing[p_missing > 0], decreasing = TRUE)
print(pm_sorted)

# We'll save it again now 
write.table(data_filled, file="imputed_data_V3.csv", sep = ",", row.names = F)
